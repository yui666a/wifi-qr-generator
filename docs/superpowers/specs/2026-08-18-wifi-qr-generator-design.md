...
- ステルス SSID の指定
- QR コードの表示と PNG ダウンロード（中央に Wi-Fi 公式ロゴを配置）
- 印刷用カードの出力（QR とネットワーク名・パスワードを併記）

含めないもの。

- QR の色や任意ロゴの差し替え（読み取り率の検証コストに見合わない）
...
ペイロードを QR 画像に変換し、中央に Wi-Fi 公式ロゴを合成する。
SVG は data URL として返し、`<img>` で読み込む。
文字列のまま DOM へ流し込むと `dangerouslySetInnerHTML` が必要になるため。

誤り訂正レベルは H。ロゴが覆う分のセルが失われるため、M では中央を隠した時点で
読み取れなくなる余地が大きい。

ロゴが覆う一辺の割合は 0.2。同条件のデコード検証では 0.30 まで読め 0.35 で
読めなくなるため、限界の半分強にあたる値を採る。印刷の掠れや斜めからの読み取りで
実効的な復元能力は落ちるので、限界近くまで大きくはしない。

ロゴの下には白地を敷く。QR の黒セルとロゴが直接触れると輪郭が潰れるため。

ロゴは Wi-Fi Alliance がライセンスフリーで配布している Wi-Fi Cube Logo の
ベクター版（PDF）を SVG に変換して使う。黒地と白抜き文字の 2 色構成なので、
単色に潰すとロゴが黒い塊になる。
...
さらに、ロゴが覆う領域を白で塗り潰した画像がデコードできることを確かめる
（`qrLogo.test.ts`）。誤り訂正で復元できる範囲に収まっているかは、実際に
デコードしないと分からないため。
